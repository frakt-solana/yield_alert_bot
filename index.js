import {generateAlertCardFile, removeCardFile, generateCardFilePath} from './generateCard/index.js'
import {createPostOnDiscordChannel, initDiscord, client, createEmbed} from './discord/index.js'
...
const programId = new frakt.web3.PublicKey('A66HabVL3DzNzeJgcHYtRRNW1ZRMKwBfrdSR4kLsZ9DJ');
...
const main = async () => {
    try {
        ...
        const allInfo = await frakt.loans.getAllProgramAccounts(programId, conn)
        // Set variables for first run
        if(first){
            allInfo.priceBasedLiquidityPools.forEach(pool=>{
                let collectionName = collectionsID[`${pool.liquidityPoolPubkey}`]
                let depositAPR = pool.depositApr/100
                depositYield[`${collectionName}`] = depositAPR
            })
            first = false
        }
        // Compare last run with current run & check if trigger  
        allInfo.priceBasedLiquidityPools.forEach(async pool=>{
            let collectionName = collectionsID[`${pool.liquidityPoolPubkey}`]
            let currentAPR = pool.depositApr/100
            let pastAPR = depositYield[`${collectionName}`]
            let trigger = pastAPR+percent
            if(currentAPR >= trigger){
                let message = `<@&${roleID}>`
                await generateAlertCardFile(pool.id, {
                    collectionName:collectionName,
                    collectionImageUrl: collectionsURL[`${collectionName}`],
                    pastAPR: pastAPR,
                    currentAPR: currentAPR
                })
                let fullPathToCardImage = generateCardFilePath(pool.id)
                await createPostOnDiscordChannel(channelID, message, fullPathToCardImage)
                await removeCardFile(pool.id, 30000)
            }
